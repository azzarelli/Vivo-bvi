...
class VideoTrainer(BaseTrainer):
    ...
    def validate_dataset_poses(self):
        ...
        for img_idx, data in enumerate(dataset):
            if img_idx % 300 == 0:
                rays_o = data['rays_o']  # (N, 3) torch tensor
                rays_d = data['rays_d']  # (N, 3) torch tensor

                # Select the first ray origin
                selected_rays_o = rays_o[0].unsqueeze(0)  # Shape (1, 3)

                # Compute the average ray direction
                avg_ray_d = rays_d.mean(dim=0, keepdim=True)  # Shape (1, 3)

                # Compute ray endpoint (extend ray for visualization)
                ray_length = 0.1  # Adjust this for visualization scale
                ray_endpoint = selected_rays_o + (avg_ray_d * ray_length)

                # Convert to numpy for plotting
                selected_rays_o = selected_rays_o.cpu().numpy()
                ray_endpoint = ray_endpoint.cpu().numpy()

                # Add ray to plotly figure
                fig.add_trace(go.Scatter3d(
                    x=[selected_rays_o[0, 0], ray_endpoint[0, 0]],
                    y=[selected_rays_o[0, 1], ray_endpoint[0, 1]],
                    z=[selected_rays_o[0, 2], ray_endpoint[0, 2]],
                    mode='lines',
                    line=dict(color='red', width=3),
                    showlegend=False
                ))

                print(f"Processed image {img_idx}: {rays_o.shape}, {rays_d.shape}")
                pb.update(1)
        pb.close()
        fig.update_layout(
