    ...
    def get_rand_ids(self, index):
        ...
        if self.sampling_weights is not None:
            ...
            if num_weights > self.sampling_batch_size:
                ...
                # TODO: Why the fuck are the IST weights returning a sum of zero :(((((((
                probs = self.sampling_weights[subset].float()
                if probs.sum() == 0:
                    probs = torch.ones_like(probs)  # Assign equal probability
                probs = probs / probs.sum()

                samples = torch.multinomial(
                    input=probs, num_samples=batch_size)
