...
class GUI:
    ...
    def __init__(self,
        ...
        try:
            self.W, self.H = self.scene.getTestCameras()[0][0].image_width, self.scene.getTestCameras()[0][
                0].image_height
    ...
    def render(self):
        ...
        if self.gui:
            ...
            while dpg.is_dearpygui_running():
                ...
                if (self.iteration % self.args.test_iterations) == 0 or (
                        self.iteration == 1 and self.stage == 'fine' and self.opt.coarse_iterations > 50):
                    self.test_step()
    ...
    def train_step(self):
        ...
        for viewpoint_cam in viewpoint_cams:
            ...
            render_pkg = render(viewpoint_cam, self.gaussians, self.pipe, self.background, stage=self.stage,
                                cam_type=self.scene.dataset_type)
        ...
        if self.hyperparams.time_smoothness_weight != 0 and self.stage == 'fine':
            loss += self.gaussians.compute_regulation(self.hyperparams.time_smoothness_weight,
                                                      self.hyperparams.l1_time_planes, self.hyperparams.plane_tv_weight)
        ...
        with torch.no_grad():
            ...
            if self.iteration < self.opt.densify_until_iter:
                ...
                if self.stage == "coarse":
                    ...
                    opacity_threshold = self.opt.opacity_threshold_fine_init - self.iteration * (
                                self.opt.opacity_threshold_fine_init - self.opt.opacity_threshold_fine_after) / (
                                            self.opt.densify_until_iter)
                    densify_threshold = self.opt.densify_grad_threshold_fine_init - self.iteration * (
                                self.opt.densify_grad_threshold_fine_init - self.opt.densify_grad_threshold_after) / (
                                            self.opt.densify_until_iter)

                if self.iteration > self.opt.densify_from_iter and self.iteration % self.opt.densification_interval == 0 and \
                        self.gaussians.get_xyz.shape[0] < 360000:
                    size_threshold = 20 if self.iteration > self.opt.opacity_reset_interval else None
                    self.gaussians.densify(densify_threshold, opacity_threshold, self.scene.cameras_extent,
                                           size_threshold, 5, 5, self.scene.model_path, self.iteration, self.stage)

                if self.iteration > self.opt.pruning_from_iter and self.iteration % self.opt.pruning_interval == 0 and \
                        self.gaussians.get_xyz.shape[0] > 200000:
                    size_threshold = 20 if self.iteration > self.opt.opacity_reset_interval else None
                    self.gaussians.prune(densify_threshold, opacity_threshold, self.scene.cameras_extent,
                                         size_threshold)

                # if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 :
                if self.iteration % self.opt.densification_interval == 0 and self.gaussians.get_xyz.shape[
                    0] < 360000 and self.opt.add_point:
            ...
            if (self.iteration in self.checkpoint_iterations):
                ...
                torch.save((self.gaussians.capture(), self.iteration),
                           self.scene.model_path + "/chkpnt" + f"_{self.stage}_" + str(self.iteration) + ".pth")
    ...
    def test_step(self):
        ...
        if self.iteration < (self.final_iter - 1) and (self.iteration % 500) != 0:
            ...
            while idx < 10:
                ...
                viewpoint_cams.append(self.test_viewpoint_stack[randint(0,
                                                                        len(self.test_viewpoint_stack) - 1)])  # TODO: perhaps ensuring varying view positions rather thn just random views
                idx += 1
        else:
...
if __name__ == "__main__":
    ...
    gui = GUI(args=args, hyperparams=hp.extract(args), dataset=lp.extract(args), opt=op.extract(args),
              pipe=pp.extract(args), testing_iterations=args.test_iterations, saving_iterations=args.save_iterations,
              ckpt_it=args.checkpoint_iterations, ckpt_start=args.start_checkpoint, debug_from=args.debug_from,
