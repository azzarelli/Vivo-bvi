...
def render_no_train(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0,
                    override_color=None, basicfunction=None, GRsetting=None, GRzer=None, cams_pc=None):
    """
    Render the scene.
    ...
    pointtimes = torch.ones((pc.get_xyz.shape[0], 1), dtype=pc.get_xyz.dtype, requires_grad=False,
                            device="cuda") + 0  #
    try:
    ...
    means3D = means3D + pc._motion[:, 0:3] * tforpoly + pc._motion[:, 3:6] * tforpoly * tforpoly + pc._motion[:,
                                                                                                   6:9] * tforpoly * tforpoly * tforpoly
    rotations = pc.get_rotation(tforpoly)  # to try use
    colors_precomp = pc.get_features(tforpoly)

    # if cams_pc is not None:
    #     xyzs = cams_pc['xyzs'].to(means3D.device) + 0.
    #     rot = cams_pc['qs'].to(means3D.device) + 0.
    #
    #     op = torch.ones_like(xyzs[:, 0], requires_grad=True).to(opacity.device).unsqueeze(-1) + 0.
    ...
    #     means3D = torch.cat([means3D, xyzs], dim=0)
    #     scales = torch.cat([scales, scaling], dim=0)
    #     opacity = torch.cat([opacity, op], dim=0)
    #     # shs = torch.cat([shs, sh], dim=0)
    #     rotations = torch.cat([rotations, rot], dim=0)
    ...
    rendered_image = pc.rgbdecoder(rendered_image.unsqueeze(0), viewpoint_camera.rays,
