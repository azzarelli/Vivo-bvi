...
class CondenseData(Dataset):
    def __init__(
        ...
        self.image_paths, self.image_poses, self.image_times, self.fovs, self.CC, self.TIP = self.load_images_path(self.root_dir, self.split)
    ...
    def load_images_path(self, cam_folder, split,  stage='fine'):
        ...
        time_dict_pointer = {id:[] for id in range(0, 300)}
        ...
        for cam_info in self.cam_infos:
            ...
            fp = os.path.join(cam_folder, f"{split}/{cam_info}/color_corrected") #color_corrected/") # masks_final
            ...
            for img_fp in os.listdir(fp):
                ...
                time_dict_pointer[int(img_fp.split('.')[0])].append(index)
                cnt+= 1
                index += 1

        self.num_frames = cnt
        return image_paths, image_poses, image_times, FOVs, CCs, time_dict_pointer
    ...
    def load_cc(self, index):
        return self.CC[index]