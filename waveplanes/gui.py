...
class GUI:
    ...
    def train_step(self):
        ...
        for viewpoint_cam in viewpoint_cams:
            ...
            render_pkg_LR = render(viewpoint_cam, self.gaussians, self.pipe, self.background, stage='fine_LR',cam_type=self.scene.dataset_type)
            image_LR= render_pkg_LR["render"]
            LR_images.append(image_LR.unsqueeze(0))
        ...
        LR_tensor = torch.cat(LR_images,0)
	...
	# Low resolution loss as well
	
        lr_loss = l1_loss(LR_tensor, gt_image_tensor[:,:3,:,:])
